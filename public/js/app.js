...
window.jQuery(function ($) {
      ...
      var submitForm = function submitForm(type) {
        ...
        window.axios.post($form.attr('action'), formData, {
          ...
          if (response && !response.data.error) {
            ...
            var subsidiaries = response.data.subsidiaries;
            ...
            $.map(subsidiaries, function (subsidiary, key) {
              var $subsidiary = $("input[type=\"hidden\"][name=\"subsidiaries[".concat(key, "][id]\"]"));
              var $subsidiaryAddress = $("input[type=\"hidden\"][name=\"subsidiaries[".concat(key, "][address][id]\"]"));
              $subsidiary.val(subsidiary.id);
              $subsidiaryAddress.val(subsidiary.address_id);
              $.map(subsidiary.cnaes, function (cnae, cnaeKey) {
                var $cnae = $("input[type=\"hidden\"][name=\"subsidiaries[".concat(key, "][cnaes][").concat(cnaeKey, "][id]\"]"));
                $cnae.val(cnae.id);
              });
            });
            ...
            if (url) {
              ...
              $alert.find('ul').html('' + '<li>' + 'Todos os dados informados foram transmitidos com sucesso e a sua solicitação já está em andamento. Para consultar seu processo, clique em consulte seu processo na barra lateral.' + '</li>');
              ...
              if (type === 'finished') {
                var _$template = $('#alert-template');

                var _$alert = _$template.clone().find('.alert');

                _$alert.toggleClass('alert-warning');
                ...
                _$alert.find('ul').html($message);

                $('.box-body .box-alerts').html('').prepend(_$alert);
                window.scroll({
                  top: 0,
                  left: 0,
                  behavior: 'smooth'
    ...
    if ($buttonAddNewCnae.length) {
      $buttonAddNewCnae.on('click', function (e) {
        ...
        $contents.find('input').each(function (index, element) {
          var $current = $(element);
          var oldStr = $current.attr('name');
          var newStr = oldStr.replace('[cnaes][]', "[cnaes][".concat(newId, "]"));
          $current.attr({
            'id': newStr,
            'name': newStr
          });
  ...
  function addEventButtonRemoveSubsidiary() {
    var $buttonRemoveSubsidiary = $('#subsidiaries').find('[data-button-remove-subsidiary]');
    debugger;
    ...
    if ($buttonRemoveSubsidiary.length) {
      $buttonRemoveSubsidiary.on('click', function (e) {
        ...
        debugger;
        var subsidiaryId = $button.data('button-remove-subsidiary');
        debugger;
        ...
        window.axios["delete"]("/dashboard/process/subsidiary/".concat(subsidiaryId));
        debugger;
      });
    }
  }
  ...
  if ($buttonAddNewSubsidiary.length) {
    $buttonAddNewSubsidiary.on('click', function (e) {
      e.preventDefault();
      var $newSubsidiaryTemplate = $('#new-subsidiary');
      var $subsidiariesContainer = $('#subsidiaries');
      ...
      $contents.find('#tab-content-subsidiary-').find('input, textarea, select').each(function (index, element) {
        var $current = $(element);
        var oldStr = $current.attr('name');
        var newStr = oldStr.replace('subsidiaries[]', "subsidiaries[".concat(newId, "]"));
        $current.attr({
          'id': newStr,
          'name': newStr
        });
  ...
  (function initEvents() {
    ...
    $(document).on('change', 'select[name*="job_roles"]', function (e) {
      e.preventDefault();
      var JOB_ROLES_OTHER = '6';
      var JOB_ROLES_REPRESENTATIVE = '3';
    ...
    $(document).on('change', 'select[name*="establishment_has_avcb_clcb"]', function (e) {
      e.preventDefault();
      $(this).closest('.form-group').toggleClass('col-md-12 col-md-6');
      $(this).closest('.row').find('input[name*="avcb_clcb_number"]').prop('disabled', function (key, value) {
        return !value;
      }).closest('.form-group').toggleClass('hidden');
      $(this).closest('.row').find('select[name*="avcb_clcb_number_type"]').prop('disabled', function (key, value) {
        return !value;
      }).closest('.form-group').toggleClass('hidden');
    });
    ...
    var DEFAULT_LABEL_SHARE_CAPITAL = 'Capital Social';

    function resetSubsidiaryFields(subsidiaryFields) {
      Object.keys(subsidiaryFields).map(function (field) {
        if (field === 'cnaes') {
        ...
        if (field === 'share_capital') {
          subsidiaryFields[field].closest('.form-group').find('label').html(DEFAULT_LABEL_SHARE_CAPITAL);
        }
    ...
    $(document).on('change', 'select[name*="request"]', function (e) {
      e.preventDefault();
      var subsidiaryFields = subsidiaryFieldsFunction($(this));
      resetSubsidiaryFields(subsidiaryFields);
      var REQUEST_OPENING = '1';
      var REQUEST_CANCELING = '2';
      var REQUEST_CHANGING = '3';
      ...
      if (REQUEST_OPENING === request) {
        ...
        subsidiaryFields.share_capital.closest('.form-group').find('label').html(DEFAULT_LABEL_SHARE_CAPITAL + ' (Não havendo destaque de capital, favor preencher 0,00)');
    ...
    $(document).on('change', 'select[name*="fields_changed"]', function (e) {
      ...
      resetSubsidiaryFields(subsidiaryFields);
      var FIELDS_CHANGED_ACTIVITY = '1';
      var FIELDS_CHANGED_ADDRESS = '2';
      var FIELDS_CHANGED_CAPITAL = '3';
      var fieldsChanged = $(this).val();
    ...
    $(document).on('change', 'select[name*="fields_editing"]', function (e) {
      e.preventDefault();
      var isCompany = $.inArray('company', $(this).val()) !== -1;
      $(this).closest('form').find('.form-group.description_of_changes').toggleClass('hidden', !isCompany).find('textarea[name="description_of_changes"]').prop('disabled', !isCompany);
    });
    $(document).on('blur', '.postcode', function () {
      ...
      var $complement = $address.find(".complement");
...
__webpack_require__(/*! /home/agily/projects/rafael/gcr-client/resources/js/app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! /home/agily/projects/rafael/gcr-client/resources/sass/app.scss */"./resources/sass/app.scss");
